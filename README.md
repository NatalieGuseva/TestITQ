...
├── index.html                  # веб-интерфейс (открыть в браузере)
...
Файл `index.html` в корне репозитория — готовый веб-интерфейс для работы с сервисом.

**Как запустить:**
1. Убедитесь что сервис запущен на `http://localhost:8080`
2. Откройте файл `index.html` в браузере (двойной клик или перетащите в окно браузера)
3. В левом нижнем углу должно появиться **«API работает»**

**Возможности интерфейса:**
- Дашборд со статистикой по статусам документов
- Список документов с фильтрацией по статусу и автору
- Создание документа
- Просмотр документа с историей изменений
- Batch Submit и Batch Approve по списку ID
- Тест конкурентного утверждения

> **Примечание:** `index.html` обращается к `localhost:8080` напрямую из браузера.
> CORS настроен в `CorsConfig.java` — никаких дополнительных настроек не требуется.

### 4. Запустить утилиту генерации
